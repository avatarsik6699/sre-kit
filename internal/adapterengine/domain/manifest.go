...
func (m Manifest) Validate() error {
	...
	if len(m.PresentationSchema) > 0 && !json.Valid(m.PresentationSchema) {
		return fmt.Errorf("adapterengine: manifest %q: presentation_schema must be valid JSON", m.Name)
	}
	switch m.Mode {
	...
	default:
		return fmt.Errorf("adapterengine: manifest %q: mode must be %q, %q or %q, got %q", m.Name, ModePull, ModeStream, ModePush, m.Mode)
	}
	return nil
}
